...
SELECT DISTINCT emp_id, salary FROM salaries
...
SELECT DISTINCT emp_id, salary FROM salaries
WHERE (salary BETWEEN 60000000 AND 70000000) AND end_at IS NULL;
...
SELECT title_code, title FROM titles
WHERE title LIKE '%사%';
...
SELECT emp_id, AVG(salary) AS avg_sal FROM salaries
GROUP BY emp_id;
...
SELECT emp_id, AVG(salary) avg_sal FROM salaries
GROUP BY emp_id
...
SELECT emp.emp_id, emp.`name`, emp.gender FROM employees emp
WHERE emp.emp_id IN (
    SELECT emp_id, end_at FROM salaries
    GROUP BY emp_id
...
WHERE emp_id IN (
    SELECT emp_id FROM title_emps
    WHERE title_code = 'T005' AND end_at IS NULL
);